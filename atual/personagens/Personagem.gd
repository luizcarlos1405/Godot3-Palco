...
func _physics_process(delta):
	...
	# Se o personagem estivere no chão
	if is_on_floor():
		# Zeramos a velocidade em 0
		velocidade_atual.y = 0
		# E se ele estiver pressinando o botão para pular e está no chão
		if pular_pressionado:
			# Aplicamos a velocidade de pulo
			velocidade_atual.y = velocidade_pulo
	
	# Se o personagem bater a cabeça no teto dua velocidade também é zerada
	if is_on_ceiling():
		velocidade_atual.y = 0
	
	""" Estas duas linhas aplicam a aceleração da gravidade no personagem
	e fazem com que ele tenda à velocidade de queda. """
	var variacao = aceleracao_gravidade * delta
	""" Se o botão de pular não estiver sendo pressoinado e a velocidade
	em y estiver negativa (personagem indo pra cima), aplicamos uma
	gravidade mais forte multiplicando pelo fator de pulo. """
	if not pular_pressionado and velocidade_atual.y < 0:
		variacao = variacao * fator_forca_pulo
